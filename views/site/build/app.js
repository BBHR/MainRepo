...
/***/ function(module, exports) {
	...
	var controller = function($scope, $rootScope, $http, $timeout,authProvider) {
	  var self = this;
	    ...
	    $http({
	      method: 'POST',
	      url: 'http://bibihelpertest.ru.swtest.ru/web/data/index',
	    })
	    .then(function(response){
...
/***/ function(module, exports) {
	...
	var controller = function(authProvider,$scope,$routeParams,$location,$http,$timeout) {
	  ...
	  var request = {
	      method: 'POST',
	      url: 'http://bibihelpertest.ru.swtest.ru/web/auth/confirm',
	      data: JSON.stringify({
	          key: auth_key
	      })
	  };
...
/***/ function(module, exports) {
	...
	var controller = function($scope,$http,$location) {
	  ...
	  this.restore = function() {
	    ...
	    var request = {
	        method: 'POST',
	        url: 'http://bibihelpertest.ru.swtest.ru/web/auth/restore',
	        data: JSON.stringify({
	            email: self.email
	        })
	    };
	    ...
	    return {
	        ...
	        link : function(scope,element,attrs) {
	          ...
	          $(document).bind('click', function(event){
	            ...
	            var marker = false;
	            while (node !== null) {
	              var check = [].filter.call(angular.element(element[0]).children(),function(item) {return item == node;}  );
	              if (check.length > 0) {
	                marker = true;
	                node = null;
	                continue;
